...
### Money model
There are **no payment cards**. Transactions reference a `bank_account_id`
directly and carry their own `currency`. Do not reintroduce a cards concept.
...
`transactions`, `transfers`, `financial_goals`, `goal_contributions`, `cars`,
`car_members`, `fuel_entries`, `pets`, `pet_health_records`, `pet_measurements`,
`homes`, `shopping_lists`, `shopping_list_items`, `shopping_sessions`,
`appointments`, `appointment_pets`, `notifications`, `notification_settings`,
`device_tokens`, `scheduled_notifications`, `app_versions`.
...
- `transactions`: `bank_account_id` (required), `transaction_source_id`,
  `category_id`, `amount`, `currency`, `type` (income|expense|adjustment),
  `latitude`/`longitude`, `pet_id`/`car_id`/`home_id`. NO `payment_card_id`.
...
- `CupertinoPushedRouteShell` — pushed-route chrome; pass `onRefresh` for pull-to-refresh.
- `SliverPushedRouteShell` — entity detail pages with collapsing header (`onRefresh` supported).
