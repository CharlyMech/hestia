...
class AddEditCardScreen extends StatefulWidget {
  ...
  /// Account members for the owner picker (shared accounts).
  /// Pass an empty list for personal accounts.
  final List<AccountMember> accountMembers;

  const AddEditCardScreen({
    super.key,
    required this.account,
    required this.accountMembers,
    this.existing,
  });
...
class _AddEditCardScreenState extends State<AddEditCardScreen> {
  ...
  AccountMember? get _selectedMember => widget.accountMembers
      .where((m) => m.userId == _ownerProfileId)
      .firstOrNull;
  ...
  PaymentCard get _preview => PaymentCard(
        ...
        ownerName: _selectedMember?.userName,
      );
  ...
  void initState() {
    ...
    if (c != null) {
      ...
      _ownerProfileId = c.ownerProfileId;
    }
  }
      ...
      child: ListView(
        ...
        children: [
          ...
          if (widget.accountMembers.isNotEmpty) ...[
            ...
            const SizedBox(height: 8),
            _OwnerPicker(
              members: widget.accountMembers,
              selectedUserId: _ownerProfileId,
              surface: surface,
              border: border,
              fg: fg,
              muted: muted,
              onChanged: (userId) =>
                  setState(() => _ownerProfileId = userId),
            ),
            const SizedBox(height: 16),
          ],
            ...
            child: GestureDetector(
              ...
              child: Container(
                padding: const EdgeInsets.symmetric(
                    horizontal: 16, vertical: 10),
                decoration: BoxDecoration(
                  color: selectedUserId == null ? fg : surface,
                  borderRadius: BorderRadius.circular(22),
                  border: Border.all(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(22),
                    border:
                        Border.all(color: selected ? fg : border),
                  ),
                  child: Text(
                    m.userName ?? m.userId.substring(0, 8),
                    style: AppFonts.body(
                      fontSize: 12,
                      fontWeight: FontWeight.w600,
                      color:
                          selected ? CupertinoColors.white : fg,
                    ),
                  ),
                ),
              ),
